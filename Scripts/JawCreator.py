...
def createGuides(number = 6):
    ...
    lef_corner_loc = mc.spaceLocator(name='{}_{}Corner_lip_{}'.format(LEFT, JAW, GUIDE))[0]
    right_corner_loc = mc.spaceLocator(name='{}_{}Corner_lip_{}'.format(RIGHT, JAW, GUIDE))[0]

    mc.parent(lef_corner_loc, lip_locs_grp)
    mc.parent(right_corner_loc, lip_locs_grp)

    mc.setAttr('{}.t'.format(lef_corner_loc), *(number + 1, 0, 0))
...
def connectSeal(part):
    ...
    for side in 'LR':
        ...
        delay_div_name = '{}.{}_delay_DIV'. format(side, seal_token)
        ...
        for index in range(value):
            ...
            #create SUB node
            delay_sub_name = '{}_{}_{}_SUB'.format(index_name, side, seal_token)
            delay_sub = mc.createNode('plusMinusAverage', name=delay_sub_name)
    ...
    for left_index, const_target in enumerate(const_targets):
        ...
        # connect left attribute to input of remap
        ...
        r_remap = createNode('remapValue', name=r_remap_name)
        ...
        clamp = createNode('remapValue', name=clamp_name)
...
def createJawPin():
    ...
    for side in 'LR':
        ...
        #conect driver to broad joint constraint targets
        const_pint_up = '{}_jaw_broadCorner_JNT_OFF_parentConstraint1.C_jaw_broadUpper_JNT_OFFW0'.format(side)
        const_pint_down = '{}_jaw_broadCorner_JNT_OFF_parentConstraint1.C_jaw_broadLower_JNT_OFFW1'.format(side)

        mc.connectAttr('{}.{}_pin'.format(pin_driver, side), const_pint_up)

        rev = mc.createNode('reverse', name='{}_corner_pin_REV'.format(side))
        mc.connectAttr('{}.{}_pin'.format(pin_driver, side), '{}.inputX'.format(rev))
        mc.connectAttr('{}.outputX'.format(rev), const_pint_down)
